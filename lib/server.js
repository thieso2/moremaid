...
async function startFolderServer(folderPathOrFS, isTemp = false, selectedTheme = null, keepRunning = false) {
    ...
    const autoCleanup = !keepRunning; // Auto-cleanup by default unless --keep-running is specified
...
<body data-typography="default">
    <div class="container">
        ...
        <div class="path">${baseDir}</div>
            ...
            const searchQuery = parsedUrl.query.search || null;
            if (!file) {
...
<body data-typography="default">
    ...
    server.listen(port, () => {
        ...
        if (autoCleanup) {
            ...
            console.log('♾️  Server will keep running (use --keep-running flag)');
        }
