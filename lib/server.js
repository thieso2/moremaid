...
async function startFolderServer(folderPath, isTemp = false, selectedTheme = null) {
    ...
    const checkForCleanup = () => {
        ...
        if (isTemp && activeConnections.size === 0) {
            console.log('🔌 WebSocket disconnected, shutting down...');
            // Immediate cleanup with frameset
            if (wss) wss.close();
            server.close();
            process.exit(0);
        }
    ...
    const server = http.createServer((req, res) => {
        ...
        if (pathname === '/') {
            // For temp mode, serve frameset wrapper
            if (isTemp && !parsedUrl.query.nowebsocket) {
                const framesetHtml = generateFramesetWrapper(port);
                res.writeHead(200, { 'Content-Type': 'text/html' });
                res.end(framesetHtml);
                return;
            }
            // Redirect to /index for regular mode or iframe
            res.writeHead(302, { 'Location': '/index' + (parsedUrl.query.nowebsocket ? '?nowebsocket=1' : '') });
            res.end();
            return;
...
<body data-typography="default">
    <div class="container">
        ...
        <div class="path">${baseDir}</div>
            ...
            // Only inject WebSocket client code if not in iframe
            if (isTemp && !parsedUrl.query.nowebsocket) {
