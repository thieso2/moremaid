...
<body data-typography="default">
    ...
    <div class="search-help">
        Press <kbd>SHIFT+TAB</kbd> to toggle search mode • <kbd>TAB</kbd> to navigate to files • <kbd>↑↓</kbd> to select • <kbd>ENTER</kbd> to open
    </div>
    ...
    <script>
        ...
        function openSelectedFile() {
            if (selectedFileIndex >= 0 && selectedFileIndex < visibleFiles.length) {
                ...
                const link = selectedItem.querySelector('a[href^="/view?file="]');
                if (link) {
        ...
        searchField.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                // Don't clear search if overlay is visible - let overlay handle ESC
        ...
        fileListFocus.addEventListener('keydown', (e) => {
            if (e.key === 'Tab' && !e.shiftKey) {
                ...
                if (selectedFileIndex > 0) {
                    ...
                    // Arrow up from first item returns to search
                    setFocusMode('search');
                }
            } else if (e.key === 'Enter') {
                e.preventDefault();
                ...
                if (!overlay.classList.contains('visible')) {
                    // ESC in list mode returns to search
                    setFocusMode('search');
                }
            }
        });
                    ...
                    fetch('/view?file=' + encodeURIComponent(filePath))
                        ...
                        .then(html => {
                            ...
                            if (content) {
                                ...
                                // Re-initialize any mermaid diagrams
                                if (typeof mermaid !== 'undefined') {
                                    mermaid.init(undefined, overlayBody.querySelectorAll('.mermaid'));
                                }
        ...
        // WebSocket connection for temp mode (if needed)
        const isTemp = window.location.hostname === 'localhost' &&
                    ...
                    if (reconnectAttempts < maxReconnectAttempts) {
                        ...
                        document.body.innerHTML = \`
                            <div style="
                                ...
                                font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
                            ">
                                <h1 style="color: #e74c3c;">Connection Lost</h1>
                                <p style="color: #666;">The server has been shut down</p>
                                <p style="color: #999; font-size: 0.9em;">This window can be closed</p>
                            </div>
                        \`;
                    }
                };
