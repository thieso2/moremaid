...
<body data-typography="default">
    <div class="search-container">
        <button class="filter-toggle${currentFilter === '*' ? ' active' : ''}" id="filterToggle" title="${currentFilter === '*' ? 'All files (click to show markdown only)' : 'Markdown only (click to show all files)'}">
            ${currentFilter === '*' ?
                `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                    <rect x="8" y="4" width="14" height="18" fill="white" stroke="currentColor" stroke-width="1.5" rx="1" opacity="0.4"/>
                    <rect x="6" y="2" width="14" height="18" fill="white" stroke="currentColor" stroke-width="1.5" rx="1" opacity="0.6"/>
                    <rect x="4" y="0" width="14" height="18" fill="white" stroke="currentColor" stroke-width="1.5" rx="1"/>
                    <polygon points="4,0 4,4 8,0" fill="var(--code-bg)" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/>
                    <circle cx="8" cy="14" r="1.5" fill="#10b981"/>
                    <circle cx="12" cy="14" r="1.5" fill="#f59e0b"/>
                    <circle cx="16" cy="14" r="1.5" fill="#8b5cf6"/>
                </svg>` :
                `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                    <rect x="4" y="0" width="16" height="20" fill="white" stroke="currentColor" stroke-width="1.5" rx="2"/>
                    <polygon points="4,0 4,5 9,0" fill="var(--code-bg)" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/>
                    <line x1="7" y1="8" x2="17" y2="8" stroke="var(--file-info-color)" stroke-width="0.8"/>
                    <line x1="7" y1="11" x2="17" y2="11" stroke="var(--file-info-color)" stroke-width="0.8"/>
                    <rect x="7" y="15" width="10" height="6" fill="#2563eb" rx="1"/>
                    <text x="12" y="19.5" font-family="monospace" font-size="5" font-weight="bold" fill="white" text-anchor="middle">MD</text>
                </svg>`
            }
        </button>
        <span class="search-mode" id="searchModeIndicator" title="Click to toggle: search in names & paths ⇄ search in file contents" data-mode="filename">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                <rect x="2" y="0" width="20" height="24" fill="none" stroke="currentColor" stroke-width="1.5" rx="2"/>
                <rect x="5" y="4" width="14" height="6" fill="var(--code-bg)" stroke="var(--file-info-color)" stroke-width="0.8" rx="1"/>
                <circle cx="16" cy="10" r="4" fill="white" stroke="#2563eb" stroke-width="1.5"/>
                <line x1="19" y1="13" x2="21" y2="15" stroke="#2563eb" stroke-width="1.5" stroke-linecap="round"/>
            </svg>
        </span>
        <input
            type="text"
    ...
    <script>
        ...
        function toggleSearchMode() {
            ...
            const modeText = searchMode === 'filename' ? 'in names & paths' : 'in file contents';

            // Update icon and tooltip
            const filenameIcon = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">' +
                '<rect x="2" y="0" width="20" height="24" fill="none" stroke="currentColor" stroke-width="1.5" rx="2"/>' +
                '<rect x="5" y="4" width="14" height="6" fill="var(--code-bg)" stroke="var(--file-info-color)" stroke-width="0.8" rx="1"/>' +
                '<circle cx="16" cy="10" r="4" fill="white" stroke="#2563eb" stroke-width="1.5"/>' +
                '<line x1="19" y1="13" x2="21" y2="15" stroke="#2563eb" stroke-width="1.5" stroke-linecap="round"/>' +
            ...
            const contentIcon = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">' +
                '<rect x="2" y="0" width="20" height="24" fill="none" stroke="currentColor" stroke-width="1.5" rx="2"/>' +
                '<line x1="5" y1="5" x2="19" y2="5" stroke="var(--file-info-color)" stroke-width="1"/>' +
                '<line x1="5" y1="9" x2="19" y2="9" stroke="var(--file-info-color)" stroke-width="1"/>' +
                '<line x1="5" y1="13" x2="19" y2="13" stroke="var(--file-info-color)" stroke-width="1"/>' +
                '<circle cx="16" cy="18" r="4" fill="white" stroke="#2563eb" stroke-width="1.5"/>' +
                '<line x1="19" y1="21" x2="21" y2="23" stroke="#2563eb" stroke-width="1.5" stroke-linecap="round"/>' +
                '</svg>';

            searchModeIndicator.innerHTML = searchMode === 'filename' ? filenameIcon : contentIcon;
            searchModeIndicator.setAttribute('data-mode', searchMode);
            searchModeIndicator.setAttribute('title', 'Click to toggle: search ' + modeText + ' ⇄ search ' + (searchMode === 'filename' ? 'in file contents' : 'in names & paths'));
        ...
        // Add click event listener to search mode indicator
        searchModeIndicator.addEventListener('click', function(e) {
            e.preventDefault();
            toggleSearchMode();
        });
        ...
        async function filterFiles(query) {
            ...
            if (searchMode === 'content' && query.length >= 2) {
                try {
                    const response = await fetch('/api/search?q=' + encodeURIComponent(query) + '&mode=content&filter=' + encodeURIComponent(currentFilter));
                    if (!response.ok) {
