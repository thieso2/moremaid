...
/**
 * Generate a frameset wrapper that maintains WebSocket connection
 */
function generateFramesetWrapper(port) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Moremaid Viewer</title>
    <style>
        body, html {
            margin: 0;
            padding: 0;
...
<body>
    <iframe id="content-frame" src="/index?nowebsocket=1"></iframe>

    <script>
        // Maintain single WebSocket connection in parent frame
        let ws = null;
        ...
        function connectWebSocket() {
            ...
            ws = new WebSocket(protocol + '//localhost:${port}/ws');

            ws.onopen = () => {
                console.log('🔗 WebSocket connected (parent frame)');
                reconnectAttempts = 0;
            };

            ws.onclose = () => {
                console.log('🔌 WebSocket disconnected - server will shut down');

                // Try to reconnect a few times in case it was temporary
                if (reconnectAttempts < maxReconnectAttempts) {
                    reconnectAttempts++;
                    console.log(\`Attempting to reconnect... (\${reconnectAttempts}/\${maxReconnectAttempts})\`);
                    setTimeout(connectWebSocket, 1000);
                } else {
                    // Show disconnection message in iframe
        ...
        // Function to get initial iframe URL based on bookmark
        function getInitialIframeUrl() {
            const hash = window.location.hash;
            if (hash) {
                // If we have a hash, load the specific file
                const file = hash.substring(1); // Remove the #
                return '/view?file=' + encodeURIComponent(file) + '&nowebsocket=1';
            }
            // Default to index
            return '/index?nowebsocket=1';
        }

        // Set initial iframe source based on bookmark
        const iframe = document.getElementById('content-frame');
        iframe.src = getInitialIframeUrl();

        // Start WebSocket connection
        connectWebSocket();

        // Handle iframe navigation to preserve WebSocket and update URL
        iframe.addEventListener('load', () => {
            try {
                // Update parent URL hash based on iframe location
                const iframeUrl = iframe.contentWindow.location.href;
                const url = new URL(iframeUrl);

                // Extract file parameter if viewing a file
                if (url.pathname === '/view' && url.searchParams.has('file')) {
                    const file = url.searchParams.get('file');
                    if (file !== lastKnownUrl) {
                        lastKnownUrl = file;
                        window.location.hash = file;
                    }
                } else if (url.pathname === '/index') {
                    // On index page, clear the hash
            ...
            // Inject a script to handle link clicks in the iframe
            try {
                const iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
                const script = iframeDoc.createElement('script');
                script.textContent = \`
                    // Override link clicks to add nowebsocket parameter
                    document.addEventListener('click', (e) => {
                        const link = e.target.closest('a');
                        if (link && link.href && link.href.startsWith(window.location.origin)) {
                            const url = new URL(link.href);
                            if (!url.searchParams.has('nowebsocket')) {
                                e.preventDefault();
                                url.searchParams.set('nowebsocket', '1');
        ...
        window.addEventListener('hashchange', () => {
            ...
            if (hash) {
                ...
                newUrl = '/view?file=' + encodeURIComponent(file) + '&nowebsocket=1';
            } else {
                newUrl = '/index?nowebsocket=1';
            }

            // Only update iframe if the URL actually changed
            try {
                ...
                if (currentUrl.pathname !== targetUrl.pathname ||
                    currentUrl.searchParams.get('file') !== targetUrl.searchParams.get('file')) {
                    iframe.src = newUrl;
                }
            } catch (e) {
                // If we can't read the iframe URL, just navigate
                iframe.src = newUrl;
            }
        });
    </script>
