    ...
    <script>
        ...
        function setupFileClickHandlers() {
            ...
            fileItems.forEach(item => {
                item.addEventListener('click', (e) => {
                    ...
                    if (e.target.classList.contains('open-new-window-btn')) {
                        e.preventDefault();
                        e.stopPropagation();
                        const filePath = e.target.dataset.path;
                        if (filePath) {
                            // Open in new window with the file hash
                            window.open(window.location.origin + window.location.pathname + '#' + filePath, '_blank');
                        }
                        return;
                    }
            ...
            // Add separate handler for buttons to ensure they work
            const openNewWindowBtns = document.querySelectorAll('.open-new-window-btn');
        ...
        function generateFileListHTML(fileData, sortMethod = 'name-asc') {
            ...
            sortedFiles.forEach(file => {
                ...
                html += \`<button class="open-new-window-btn" title="Open in new window" data-path="\${file.path}">⧉</button>\`;
