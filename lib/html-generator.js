    ...
    <script>
        ...
        function setupFileClickHandlers() {
            ...
            fileItems.forEach(item => {
                item.addEventListener('click', (e) => {
                    // Check if click was on the open-in-new-window button
                    if (e.target.closest('.open-new-window-btn')) {
                        e.preventDefault();
                        e.stopPropagation();
                        const button = e.target.closest('.open-new-window-btn');
                        const filePath = button.dataset.path;
                        if (filePath) {
                            // Open in new window with the file hash
                            const newUrl = window.location.origin + window.location.pathname + '#' + encodeURIComponent(filePath);
        ...
        function generateFileListHTML(fileData, sortMethod = 'name-asc') {
            ...
            sortedFiles.forEach(file => {
                ...
                html += \`<div class="file-name"><span>\${fullPath}</span><button class="open-new-window-btn" title="Open in new window" data-path="\${file.path}"><svg viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg></button></div>\`;
