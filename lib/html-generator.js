...
function generateIndexHtmlWithSearch(folderPath, files, directories, port, forceTheme = null) {
...
<body data-typography="default">
    <div class="search-container">
        <div class="breadcrumb-container" id="breadcrumbContainer">
            <span class="breadcrumb-item current" data-path="">📂 ${folderName}</span>
        </div>
    ...
    <script>
        ...
        const allDirectories = ${JSON.stringify(directories)};
        ...
        function getItemsInCurrentDirectory() {
            // Get files in current directory
            const filesInDir = allFiles.filter(file => {
                if (currentDirectory === '') {
                    // Root: show files with no directory or files in immediate subdirectories
                    return file.directory === '' || file.directory === '.';
                } else {
                    // Show files in this exact directory
                    return file.directory === currentDirectory;
                }
            });

            // Get immediate subdirectories
            const subdirectories = allDirectories.filter(dir => {
                if (currentDirectory === '') {
                    // Root: show top-level directories only
                    return !dir.includes('/');
                } else {
                    // Show immediate subdirectories
                    const prefix = currentDirectory + '/';
                    return dir.startsWith(prefix) && !dir.substring(prefix.length).includes('/');
                }
            });

            return { files: filesInDir, directories: subdirectories };
        }
        ...
        function updateBreadcrumb() {
            ...
            rootItem.className = 'breadcrumb-item' + (currentDirectory === '' ? ' current' : '');
            rootItem.textContent = '📂 ${folderName}';
            rootItem.dataset.path = '';
            if (currentDirectory !== '') {
                rootItem.addEventListener('click', () => navigateToDirectory(''));
            }
            ...
            if (currentDirectory !== '') {
                ...
                segments.forEach((segment, index) => {
                    ...
                    segmentItem.className = 'breadcrumb-item' + (isLast ? ' current' : '');
                    segmentItem.textContent = segment;
                    segmentItem.dataset.path = accumulatedPath;

                    if (!isLast) {
                        const pathToNavigate = accumulatedPath;
                        segmentItem.addEventListener('click', () => navigateToDirectory(pathToNavigate));
                    }
        ...
        function updateFileList() {
            const { files, directories } = getItemsInCurrentDirectory();
            const fileListEl = document.getElementById('fileList');

            // Generate combined list with directories first, then files
            fileListEl.innerHTML = generateFileListHTML([...directories.map(dir => ({
                path: dir,
                ...
                modifiedDisplay: '',
                modifiedFull: ''
            })), ...files], currentSortMethod);

            // Re-insert the focus element
        ...
        async function filterFiles(query) {
            if (!query) {
                // When no query, return files in current directory
                const { files } = getItemsInCurrentDirectory();
                return files;
            }

            // Content search (via API) - searches all files regardless of directory
            if (searchMode === 'content' && query.length >= 2) {
                try {
                    ...
                    // Filter results to current directory if not at root
                    if (currentDirectory === '') {
                        return results;
                    }
                    return results.filter(file => {
                        return file.directory === currentDirectory ||
                               file.directory.startsWith(currentDirectory + '/');
                    });
                } catch (error) {
            ...
            // Filename search (local) - searches within current directory and subdirectories
            const lowerQuery = query.toLowerCase();
            let filesToSearch = allFiles;

            // If in a subdirectory, only search within that directory tree
            if (currentDirectory !== '') {
    ...
    function generateFileListHTML(fileData, sortMethod = 'name-asc') {
        ...
        sortedFiles.forEach(file => {
            ...
            const displayName = isDirectory ? file.fileName : fullPath;
            const nameClass = isDirectory ? 'file-name is-directory' : 'file-name';

            html += `<div class="file-item" data-path="${file.path}" data-is-directory="${isDirectory}">`;
            html += '<div class="file-item-row">';
