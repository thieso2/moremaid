...
class VirtualFS {
    ...
    listDirectories() {
        throw new Error('listDirectories must be implemented');
    }
...
class DiskFS extends VirtualFS {
    ...
    listMarkdownFiles(dir = this.basePath, baseDir = this.basePath) {
        ...
        for (const item of items) {
            ...
            if (stat.isDirectory()) {
                files = files.concat(this.listMarkdownFiles(fullPath, baseDir));
    ...
    listDirectories() {
        ...
        const traverse = (dir, baseDir = this.basePath) => {
            if (!fs.existsSync(dir)) {
                return;
            }
...
class ZipFS extends VirtualFS {
    ...
    listMarkdownFiles() {
        const mdFiles = this.entries
            .filter(entry => !entry.directory && entry.filename.match(/\.(md|markdown)$/i))
    ...
    listDirectories() {
        ...
        this.entries
            ...
            .forEach(entry => {
                ...
                const dir = entry.filename.replace(/\/$/, '');
                if (dir) {
                    directories.add(dir);
                }
            });
